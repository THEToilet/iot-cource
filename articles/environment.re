...
可能であれば寄付もできますが、@<strong>{JUST DOWNLOAD}を選択することでつぎの画面に遷移します。

//image[2][寄付金の金額選択画面][scale=0.8]{
//}

@<strong>{JUST DOWNLOAD}を選択するとブラウザ内でMicorsoftStoreの画面に遷移します（@<img>{3}）。
つぎに@<strong>{入手}を選択すると、ブラウザのポップアップが表示されWindows上でMicrosoftStoreを開く許可を求められるので許可を選択してください。
...
Arduino IDEにてESP32を使うために必要なボードマネージャーのインストール方法を紹介します。
...
最初のうちは、回路的につながっている黄色の部分を忘れて、ショートする回路を作ってしまうことがあるので注意してください。
