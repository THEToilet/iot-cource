...
LEDには極性があり、以下の二つに分けられます(@<img>{LED-2})。
...
#@# // TODO : 板の大きさでアノードとカソード決めるの危険

//image[LED-2][LEDの端子の違い][scale=0.7]{
//}

ちなみに電流を流しすぎると下記の画像（@<img>{P-3}）の右側LEDのように燃えて使えなくなってしまいます。

//image[P-3][燃えてしまったLED][scale=0.6]{
//}
　
極性のほかにも点灯のために必要な情報があり、
代表的なものに以下の二つがあります。
...
60Ωが必要ということが分かりました。しかし、今回は100Ωの抵抗を用意したので
...
//image[z][ジャンプワイヤ オス・オス][scale=0.7]{
//}
//image[zyanpu_mesu][ジャンプワイヤ オス・メス][scale=0.7]{
//}

=== 抵抗
電子回路上を流れる電流を調整するために使われるのが抵抗であり（@<img>{regi330}）、抵抗値によって様々なものがあります。
これを見分けるための規格があり以下の表のように決められています（@<table>{regi}）。
//image[regi330][330Ω抵抗][scale=0.8]{
//}
...
次に右側の二つ線の色を見ると茶色と金色なので乗数が10許容差が±5%ということが分かります。
これらのことから、この抵抗は抵抗値330Ωで許容差±5%だということが分かりました。
//image[colorcode][抵抗値の読み取り]{
//}
...
まず@<img>{pullupdown}の左側に記載してあるPULL UPは常にOUT PUTに電圧をかけ、スイッチが押された際にはOUT PUTへの電圧が0になる回路です。
これによりノイズの影響を少なくしています。

つぎに@<img>{pullupdown}の右側に記載してあるPULL DOWNはOUT PUTをGNDにつないでおくことでかかる電圧を0に維持しています。スイッチが押された
際には電圧がOUT PUTにかかります。これによりノイズの影響を少なくしています。

これら二つの方法を用いることで安定した回路を組むことができます。
//image[pullupdown][PULL UPとPULL DOWN]{
//}

== Lチカしよう！
Lチカとは、さまざまな言語のプログラムでのHelloWolrdに相当するハードウェア操作のプログラムです。
LEDをチカチカ点滅させてみましょう！！

=== プログラムでLチカ
Lチカですが、ESP32を使用することで容易に実現できます。
Arduino IDEから新規作成を選択し、新たなファイルを作成して
以下のプログラムを貼り付けてください（@<list>{Ltic}）。
回路図ではブレッドボードを二枚用いていますが（@<img>{led2}）、今回使用するブレッドボードでは一枚の中に収まるようになっています（@<img>{P_20210805_181628}、@<img>{P_20210805_181635}）。
...
//image[led2][Lチカの回路図][scale=1.7]{
//}
//image[P_20210805_181628][Lチカの回路を配置した図]{
//}
//list[Ltic][Lチカのプログラム]{
void setup() {
...
//image[P_20210805_181635][LED点滅図]{
//}


=== タクトスイッチでLチカ
せっかくなので、スイッチを使用して、LEDを光らせましょう
同様に以下の画像（@<img>{switch2}、@<img>{P_20210805_182114}）を参考に電子回路を組み、プログラム（@<list>{switch}）を参考にして、
ESP32に書き込んでください。
...
//image[switch2][タクトスイッチでLチカをする回路図][scale=1.7]{
//}
//list[switch][タクトスイッチでLチカをするプログラム]{
void setup()
{
...
//image[P_20210805_182114][回路をブレッドボード上で配置した図]{
//}
//image[P_20210805_182119][スイッチでLEDを点滅させる図]{
//}
...
==== 回路図＆配置図
//image[switch3][応用問題回路図][scale=1.7]{
//}

//image[P_20210805_182414][応用問題回路配置図]{
//}

==== プログラム
//list[ltica_tica][状態遷移Lチカプログラム]{
int state = 0;
bool is_tica = false;
